...
if(file.exists("t-testBASE.html")){
  file.remove("t-testBASE.html")
}
require(learnr)
require(broom)
require(ggplot2)
...
## One sample t-tests {.tabset .tabset-pills}
...
```{r, echo=FALSE}
textInput("var_1", 
          "Choose a QUANTITATIVE variable of which you would like to make claims about the TRUE mean.", 
          value = "", 
          width = '80%', 
          placeholder = NULL)
textInput("null_1", 
...
output$check_1.0 <- renderText(checkthis(input$a_1.0, round(mean(titanic$Age), 2)))
output$check_1.1 <- renderText(checkthis(input$a_1.1, round(test()$statistic, 2)))
output$check_1.2 <- renderText(checkthis(input$a_1.2, 886))
output$check_1.3 <- renderText(checkthis(input$a_1.3, round(test()$p.value, 2)))
...
## Two-sample t-tests {.tabset .tabset-pills}
...
output$check_2.1 <- renderText(checkthis(as.numeric(input$a_2.1), round(mean(titanic$Age[titanic$Sex == "male"]), 2)))
output$check_2.2 <- renderText(checkthis(input$a_2.2, round(mean(titanic$Age[titanic$Sex == "female"]), 2)))
output$check_2.3 <- renderText(checkthis(input$a_2.3, round(test2()$statistic, 2)))
output$check_2.4 <- renderText(checkthis(input$a_2.4, round(test2()$p.value, 3)))
...
## Paired t-tests {.tabset .tabset-pills}
...
output$check_3.1 <- renderText(checkthis(as.numeric(input$a_3.1), round(mean(titanic$Siblings.Spouses.Aboard - titanic$Parents.Children.Aboard), 2)))
#output$check_3.2 <- renderText(checkthis(input$a_3.2, round(mean(titanic$Parents.Children.Aboard), 2)))
output$check_3.3 <- renderText(checkthis(input$a_3.3, round(test3()$statistic, 2)))
output$check_3.4 <- renderText(checkthis(input$a_3.4, round(test3()$p.value, 3)))
