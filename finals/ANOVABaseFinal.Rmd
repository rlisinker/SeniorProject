...
if(file.exists("ANOVABaseFinal.html")){
  file.remove("ANOVABaseFinal.html")
}
require(learnr)
require(broom)
require(ggplot2)
...
```{r, echo = TRUE}
bodwins_91 <- subset(babies, Name = c("Kelly", "Gregory", "Diane", "Jim"), Year >1990)
```
...
```{r boxplots, exercise = TRUE, exercise.lines = 5}
```
...
```{r, context = "server"}
output$check_7 <- renderText(checkthis(input$a_7, 2, 11.307))
```
...
```{r, context = "server"}
output$check_8 <- renderText(checkthis(input$a_8, 2, 0))
```
...
```{r q_10_and11}
radioButtons("a_10",
           "Order the names by count (largest count to smallest count)",
           choices = c(
             "Gregory, Kelly, Jim, Diane" = "no1",
              "Kelly, Jim, Diane, Gregory" = "no2",
             "Gregory, Kelly, Diane, Jim" = "yes",
             "Jim, Diane, Kelly, Gregory" = "no3",
             "Diane, Jim, Kelly, Gregory" = "no4"
             )
)
htmlOutput("check_10")

# could add in a fill in the blank table for connected letters report
...
output$check_11 <- renderText(
  if(input$a_11 == ""){
     ""
  }else if(input$a_11 == "yes"){
      correct
  }else{
    incorrect
  }
